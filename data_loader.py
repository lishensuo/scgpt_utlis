...
from typing import Any, Dict, List, Mapping, Optional, Tuple, Iterable, Sequence
...
class DataCollator:
    ...
    Args:
        ...
        do_mlm (:obj:`bool`): whether to do masking with MLM (Masked Language Model).
        do_binning (:obj:`bool`): whether to bin the expression values.
        mlm_probability (:obj:`float`): the probability of masking with MLM. 掩码的比例
        mask_value (:obj:`int`): the value to fill at the expression postions
            that are masked. (对于掩码值，使用 -1)
    ...
    def _call_both(
        ...
        Returns:
            Dict[str, torch.Tensor]: a dict of tensors. for one cell
            Example:
                {'pcpt_gene': tensor([[36572, 17868, ..., 17072],
                                        [36572, 17868, ..., 17072],
                                        ...,
                ...
                'gen_expr_target': tensor([[ 1.,  3., ..., 19.],
                                        ...
                                        [ 1.,  3., ..., 19.]])}
        """
        if not isinstance(examples[0], Mapping):  # 一个example就是一个细胞
            return NotImplementedError
        ...
        device = examples[0]["genes"].device
        # Calculator the max number of genes in batch cells
        # # 如果一个细胞大于这个长度，则truncation
        # # 如果一个细胞小于这个长度，则padding
        # # 由于单细胞的稀疏性，所以有较多概率小于给定的1536 
        max_ori_len = max(len(example["genes"]) for example in examples)  #每个细胞的最大基因长度
        _max_length = self.max_length if max_ori_len >= self.max_length else max_ori_len
        ...
        for i in range(len(examples)):
            ...
            # keep_first_n_tokens指的是第一个的<cls>
