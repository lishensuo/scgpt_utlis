...
handler = logging.StreamHandler(sys.stdout)  # 控制台处理器
...
class GeneVocab(Vocab):
    ...
    def _build_vocab_from_iterator(
        ...
        counter = Counter()
        counter.update(iterator)  # 统计每个基因出现的次数

        if specials is not None:
            for tok in specials:
                del counter[tok]

        #按频率排序。如频率相同情况下，按基因名排序
...
def tokenize_batch(
    ...
    Args:
        data (array-like): A batch of data, with shape (batch_size, n_features).  # batch_size个细胞，n_features个基因的表达值
            n_features equals the number of all genes.
        gene_ids (array-like): A batch of gene ids, with shape (n_features,).     # 对应n_features的id
    ...
    tokenized_data = []
    for i in range(len(data)):    # (batch_size, n_features)
        row = data[i]             # 对每行(细胞)进行处理
        mod_types = None
        if include_zero_gene:     # 是否包括表达值为0的基因
            values = row
            genes = gene_ids
            if mod_type is not None:
                mod_types = mod_type
        else:
            idx = np.nonzero(row)[0]  # 去除0表达基因
            values = row[idx]
            genes = gene_ids[idx]
            if mod_type is not None:
                mod_types = mod_type[idx]
        if append_cls:                # 在第0个位置，插入<cls>特殊词元
        ...
        # [([genes], [values], [moltypes]), ([genes], [values], [moltypes]), ...]
...
def pad_batch(
    ...
    for i in range(len(batch)):
        ...
        if len(gene_ids) > max_len:   #如果给定序列长度 超过 限定的序列长度
        ...
        if len(gene_ids) < max_len: #如果给定序列长度 小于 限定的序列长度
            gene_ids = torch.cat(
                [
                    gene_ids,
                    torch.full(
    ...
    batch_padded = {
        "genes": torch.stack(gene_ids_list, dim=0),  #拼接为一个batch的数据 (batch_size, n_genes)
