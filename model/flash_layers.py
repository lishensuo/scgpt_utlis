...
# FlashscGPTMHA首先定义mask attention注意力
## 其中会用到.layers中自定义的Multi-head attention--MultiheadAttention

# FlashscGPTLayer进一步封装Transformer块
# FlashscGPTGenerator则进一步堆叠多个块

class FlashscGPTMHA(nn.Module):
    """
    Custom MHA layer for scGPT. This takes two separate forward passes on the pect
    genes, and on the gen genes (unkown genes).
    """
    ...
    def __init__(
        ...
        self.head_dim = self.embed_dim // num_heads  #每个头分到的维度
        assert (
    ...
    def forward(
        self,
        pcpt_total_embs: Tensor,  # known genes
        gen_total_embs: Tensor,   # unknown genes
        ...
        ## 首先对pcpt(known gene)直接进行常规注意力学习
        pcpt_qkv = self.Wqkv(pcpt_total_embs) # nn.Linear(embed_dim, 3 * embed_dim) i.e. hidden_dim → 3 * hidden_dim

        pcpt_qkv = rearrange(
            pcpt_qkv, "b s (three h d) -> b s three h d", three=3, h=self.num_heads
        ) # 三维变五维 (batch_size, seq_len, 3, num_heads, head_dim)
        ...
        )  #向量的维度重排：three表示qkv，h表示注意力的多头数，d就表示最终每个q/k/v在训练时分配的嵌入维度
        ...
        cross_q = rearrange(cross_q, "b gen_s h d -> b gen_s (h d)")  # (batch, gen_len, nheads * head_dim)

        # 注意这里还是使用pcpt_qkv， 而不是pcpt_context（等下一个Transformer块会调用）
        cross_kv = torch.cat(
            [pcpt_qkv[:, :, 1:, :, :], gen_qkv[:, :, 1:, :, :]], dim=1
        )  # (batch, pcpt_seq+gen_seq, 2, nheads, head_dim) #key/value (pcpt + gen)
        ...
        def make_mask(q_len, k_len, device):
            attention_mask = torch.zeros(   #全为False。True表示忽略该注意力
        ...
        # make_mask(q_len = 3, k_len = 5)  #行表示Query，列表示K/V，True表示masked attention
        ...
        # 形状 [batch_size, seq_len(pcpt / gen)]
...
class FlashscGPTLayer(nn.Module):
    ...
    def forward(
        ...
        if self.norm_scheme == "pre":
            ...
            pcpt_total_embs2, gen_total_embs2 = self.self_attn( # FlashscGPTMHA
                pcpt_total_embs,
                gen_total_embs,
