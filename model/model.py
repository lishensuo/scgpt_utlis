...
class TransformerModel(nn.Module):
    def __init__(
        self,
        ntoken: int,            # number of tokens in vocab
        d_model: int,           # dimension of token embedding
        nhead: int,             # multi-head attention
        d_hid: int,             # hidden dimension of FFN
        nlayers: int,           # number of transformer block
        nlayers_cls: int = 3,   # number of layer for cell type prediction (not used)
        n_cls: int = 1,         # number of cell types
        vocab: Any = None,      # gene vocab
        dropout: float = 0.5,
        pad_token: str = "<pad>",
        pad_value: int = -2,
        do_mvc: bool = True,    # whether do mvc 
        do_dab: bool = False,   # whether do dab
        use_batch_labels: bool = False,
        num_batch_labels: Optional[int] = None,
        domain_spec_batchnorm: Union[bool, str] = False,
        input_emb_style: str = "continuous",        # input expreesion values
        n_input_bins: Optional[int] = None,         # number of bins
        cell_emb_style: str = "cls",                # how to calculate cell embedding, <cls> token
        mvc_decoder_style: str = "inner product",   # how to calculate mvc
        ecs_threshold: float = 0.3,                 # cell embedding similarity
        explicit_zero_prob: bool = False,           
        use_generative_training=True,               # 'both' or 'gene' (predict unknow gene based on known genes and cls emb)
        use_fast_transformer: bool = False,         # 'pcpt' transformer
        ...
        self.d_model = d_model  # use
        self.do_dab = do_dab    # not
        self.ecs_threshold = ecs_threshold  # not
        self.use_batch_labels = use_batch_labels    # not
        self.domain_spec_batchnorm = domain_spec_batchnorm  # not
        self.input_emb_style = input_emb_style  # "continuous"
        self.cell_emb_style = cell_emb_style    # <cls> toke for cell embedding
        self.explicit_zero_prob = explicit_zero_prob    # not
        self.norm_scheme = "pre" if pre_norm else "post" # False
        ...
        self.flag_encoder = nn.Embedding(2, d_model)  ## pcpt???
        ...
        if use_generative_training:
            ...
            # [MASK] 标记的查询向量会与其他词（包括其他 [MASK] 标记）的键向量计算注意力权重。
    ...
    def _encode(
        ...
        self.cur_gene_token_embs = src   # 专门为MVC设计的
    ...
    # 在scGPT-main中没有这个transformer_generate
    # generative_forward, 输出pcpt_output, gen_output
    def transformer_generate(
        ...
        if gen_genes is not None:
            gen_token_embs = self.encoder(gen_genes)  # (batch, gen_len, embsize), Only Gene id, without expression
    ...
    def forward(
        ...
        **kwargs,
        ...
        on the value of the "use_generative_training" kwarg.
        """
        if "use_generative_training" not in kwargs:
            # raise ValueError("use_generative_training kwarg is required")
            warnings.warn(
                "use_generative_training kwarg is required but not provided! "
        ...
        do_generative_training = kwargs.pop("use_generative_training")
    ...
    def generative_forward(
        ...
        # generative 的关键：基于已知的pcpt_output生成预测未知的gen
        ...
        decoder_output = self.decoder(
            ...
            if not self.use_batch_labels
                ...
                [
                    transformer_output,
                    batch_emb.unsqueeze(1).repeat(1, transformer_output.shape[1], 1), 
                    # (batch_size, hidden_dim) → (batch_size, seq_length, hidden_dim)
                    # e.g. .repeat(1, 3, 1) 对第0/2维度不变，对第1维度复制3次
                    # 最终目的是拼接表示batch的Embedding
                ],
                dim=2,
            ),
        )
        ...
        # add other keys, like output['mvc_output']
