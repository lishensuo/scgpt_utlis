...
# InitVar： 在初始化时传递参数，但不作为类实例的属性存储
from typing import Any, Dict, List, Mapping, Optional, Tuple, Union
from typing_extensions import Self, Literal

import numpy as np
from scipy.sparse import spmatrix, csr_matrix #稀疏矩阵
...
from scgpt_utlis.gene_tokenizer import GeneVocab
from .data import DataTable, MetaInfo  # data.py
from .setting import Setting    # setting.py
from . import logger  # __init__.py
...
class DataBank:
    ...
    data_tables: Dict[str, DataTable] = field(
        default_factory=dict, #每个 data_tables 实例都有一个独立的空字典作为默认值
    ...
    def gene_vocab(self, gene_vocab: Union[GeneVocab, Path, str]) -> None:
        ...
        self._gene_vocab = gene_vocab   # 前导下划线 _ 表示这是一个“受保护”的变量，建议仅在类或子类内部使用
        ...
        # 根据self.settings.immediate_save，决定运行sync还是track
    ...
    def from_anndata(
        ...
        if isinstance(to, str):
            to = Path(to)
        to.mkdir(parents=True, exist_ok=True)  # parents设置为 True，则会创建所有需要的父目录。
        db = cls(
    ...
    def batch_from_anndata(cls, adata: List[AnnData], to: Union[Path, str]) -> Self:
        raise NotImplementedError  # 提醒未实现功能
    ...
    def _tokenize(
        ...
        if isinstance(data, csr_matrix):
            indptr = data.indptr      # 数组，表示每一行的起始位置。用于快速定位行。
            indices = data.indices    # 数组，存储非零元素的列索引。
            non_zero_data = data.data # 数组，存储非零元素的实际值。

            tokenized_data = {"id": [], "genes": [], "expressions": []}    ## 核心步骤
            tokenized_data["id"] = list(range(n_rows))
            for i in range(n_rows):  # ~2s/100k cells, 遍历每个细胞的表达值
                row_indices = indices[indptr[i] : indptr[i + 1]]  #每行中的非0元素
                row_new_indices = new_indices[row_indices]        #每行中非零元素所在列的位置
                row_non_zero_data = non_zero_data[indptr[i] : indptr[i + 1]]  #每行中非元素的值
    ...
    def sync(self, attr_keys: Union[List[str], str, None] = None) -> None:
        ...
        **NOTE**: This will overwrite the existing data directory.
        ...
        if "data_tables" in attr_keys:
            # 保存每个独立的data_tables (e.g. X, counts)
